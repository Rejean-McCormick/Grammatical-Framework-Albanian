...
oper
  ...
  emptyVP : Verb -> VP = \v -> {
    ...
    perfectAux = selectPerfectAux v.morphVoice v.perfectAux ;
    cl = [] ;
    subjcl = "të" ;
    post = \\_ => []
  } ;
  ...
  realizeVP : VP -> ParamX.Tense -> Anteriority -> Polarity -> Agr -> Str =
    \vp,t,ant,pol,a -> case <ant,t> of {
      ...
      <Anter,ParamX.Fut> => negation pol ++ "do" ++
        vp.subjcl ++
        perfectAuxSubj vp.perfectAux (agrNumber a) a.p ++
        vp.participle ++ vp.post ! a ;

      <Anter,ParamX.Cond> => negation pol ++ "do" ++
        vp.subjcl ++
        perfectAuxFinite vp.perfectAux ParamX.Past (agrNumber a) a.p ++
        vp.participle ++ vp.post ! a ;

      <Anter,_> => negation pol ++ vp.cl ++
        perfectAuxFinite vp.perfectAux t (agrNumber a) a.p ++ vp.participle ++ vp.post ! a
    } ;
  ...
  realizeSubjAntVP : VP -> Anteriority -> Polarity -> Agr -> Str =
    \vp,ant,pol,a -> case ant of {
      ...
      Anter => case pol of {
        Pos => vp.subjcl ++
          perfectAuxSubj vp.perfectAux (agrNumber a) a.p ++ vp.participle ++ vp.post ! a ;
        Neg => "të" ++ "mos" ++ vp.cl ++
          perfectAuxSubj vp.perfectAux (agrNumber a) a.p ++ vp.participle ++ vp.post ! a
      }
    } ;
