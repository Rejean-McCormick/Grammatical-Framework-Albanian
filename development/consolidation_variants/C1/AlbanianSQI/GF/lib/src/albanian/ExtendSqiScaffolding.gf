...
oper
  ...
  sc_GenNP : NP -> Quant = \np -> lin Quant {
    s=\\c,g,n =>R.nyje!R.Indef!c!g!n ++ np.s!R.Ablat ;
    spec=R.Def ;
  ...
  sc_GenRP : Num -> CN -> RP = \num,cn -> lin RP {
    s=\\c,gn =>R.nyje!R.Indef!c!(cn.g ! num.n)!num.n ++ cn.s!R.Indef!R.Ablat!num.n
  } ;

  sc_GenModNP : Num -> NP -> CN -> NP = \num,np,cn -> lin NP {
    s=\\c =>cn.s!R.Def!c!num.n ++ R.nyje!R.Def!c!(cn.g ! num.n)!num.n ++ np.s!R.Ablat ;
    acc_clit=[] ; dat_clit=[] ; a=R.agrgP3 (cn.g ! num.n) num.n ; isPron=False
  } ;

  sc_GenModIP : Num -> IP -> CN -> IP = \num,ip,cn -> lin IP {
    s=\\c =>cn.s!R.Def!c!num.n ++ R.nyje!R.Def!c!(cn.g ! num.n)!num.n ++ ip.s!R.Dat ;
