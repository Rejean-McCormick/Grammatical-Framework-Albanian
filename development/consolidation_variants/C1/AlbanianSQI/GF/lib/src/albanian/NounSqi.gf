  ...
  oper
    ...
    positiveA : Adj -> Case -> Gender -> Number -> Str = \a,c,g,n ->
      case adjNeedsNyje a of {
        True  => nyje ! Indef ! c ! g ! n ++ a.s ! c ! g ! n ;
        False => a.s ! c ! g ! n
      } ;

    possSg : Str -> Str -> Str -> Case -> Str = \nom,acc,obl,c -> case c of {
      Nom => nom ; Acc => acc ; Dat => obl ; Gen => obl ; Ablat => obl
    } ;

    possPl : Str -> Str -> Case -> Str = \direct,obl,c -> case c of {
      Nom => direct ; Acc => direct ; Dat => obl ; Gen => obl ; Ablat => obl
    } ;
  ...
  lin
    DetCN det cn = {
      s = \\c => case det.placement of {
        PreNominal => det.s ! c ! (cn.g ! det.n) ++ cn.s ! det.spec ! c ! det.n ;
        PostNominal => cn.s ! det.spec ! c ! det.n ++ det.s ! c ! (cn.g ! det.n)
      } ;
    ...
    AdjCN ap cn = {
      s=\\sp,c,n=>cn.s!sp!c!n ++ ap.s!sp!c!(cn.g ! n)!n; g=cn.g
    } ;
    RelCN cn rs = {
      s=\\sp,c,n=>cn.s!sp!c!n ++ rs.s!(agrgP3 (cn.g ! n) n); g=cn.g
    } ;
    ...
    PossNP cn np = cn ** {
      s=\\sp,c,n=>cn.s!sp!c!n ++ nyje ! sp ! c ! (cn.g ! n) ! n ++ np.s!Gen
    } ;
