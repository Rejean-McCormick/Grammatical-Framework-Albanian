...
oper
  ...
  AdjClass : Type = ResSqi.AdjClass ;

  masculine : Gender = Masc ;
  feminine  : Gender = Fem ;

  articulated   : AdjClass = Articulated ;
  unarticulated : AdjClass = Unarticulated ;
  ...
  invariantCase : Str -> (Case => Str) = \x -> table {
    Nom => x ; Acc => x ; Dat => x ; Gen => x ; Ablat => x
  } ;
...
-- Strict principal-part guards. The generated two-form selectors above are
-- implementation details; public reg2* constructors must never discard a
-- supplied observable form. A fallback through reg* is acceptable only when
-- the resulting table reproduces the supplied principal part exactly.
reg2N : Str -> Str -> N = \form1,form2 ->
  let n : N = reg2NRaw form1 form2
  in case pbool2bool (Predef.eqStr (n.s ! Indef ! Nom ! Pl) form2) of {
    True  => n ;
...
reg2A : Str -> Str -> A = \form1,form2 ->
  ...
  in case pbool2bool (Predef.eqStr (a.s ! Nom ! Masc ! Pl) form2) of {
    True  => a ;
...
reg2V : Str -> Str -> V = \form1,form2 ->
  ...
  in case pbool2bool (Predef.eqStr v.participle form2) of {
    True  => v ;
    False => error "reg2V: incompatible participle principal part; use reg3V or mkVFull"
  } ;

-- Higher rungs of the public smart-paradigm ladder.  Additional principal
-- parts are validators, never hints that may be silently ignored.
reg3N : Str -> Str -> Str -> N = \lemma,plural,defNomSg ->
  let n : N = reg2N lemma plural
  in case pbool2bool (Predef.eqStr (n.s ! Def ! Nom ! Sg) defNomSg) of {
    True  => n ;
    False => error "reg3N: incompatible definite nominative singular principal part; use mkNFull"
  } ;

reg3A : Str -> Str -> Str -> A = \mascSg,mascPl,femSg ->
  let a : A = reg2A mascSg mascPl
  in case pbool2bool (Predef.eqStr (a.s ! Nom ! Fem ! Sg) femSg) of {
    True  => a ;
    False => error "reg3A: incompatible feminine singular principal part; use mkAFull"
  } ;

-- Observable verb principal parts: present 1sg, aorist 1sg, participle.
reg3V : Str -> Str -> Str -> V = \pres1sg,aor1sg,part ->
  ...
  in case pbool2bool (Predef.eqStr (v.Indicative ! Aorist ! Sg ! P1) aor1sg) of {
    True  => v ;
    False => error "reg3V: incompatible aorist principal part; use reg4V or mkVFull"
  } ;

-- Optional fourth validator for classes that also require an imperfect check.
reg4V : Str -> Str -> Str -> Str -> V = \pres1sg,aor1sg,part,imperf1sg ->
  ...
  in case pbool2bool (Predef.eqStr (v.Indicative ! Imperfect ! Sg ! P1) imperf1sg) of {
    True  => v ;
    False => error "reg4V: incompatible imperfect principal part; use mkVFull"
  } ;
...
mkNFullGenders : NForms -> Gender -> Gender -> N = \f,gsg,gpl -> lin N {
  s = table {
    Indef => table {
...
mkNFull : NForms -> Gender -> N = \f,g -> mkNFullGenders f g g ;

-- Public metadata escape hatch for ambigeneric nouns. The inflection table is
-- preserved; only singular/plural agreement gender is made explicit.
mkNAmbig : N -> Gender -> Gender -> N = \n,gsg,gpl -> setNounGender n gsg gpl ;

mkN = overload {
  mkN : Str -> N = regN;   -- indef nominative singular; compatibility/smart heuristic
  mkN : Str -> Str -> N = reg2N ;  -- indef nom sg, indef nom pl
  mkN : Str -> Str -> Str -> N = reg3N  -- + definite nom sg validator
} ;
...
mkPN = overload {
  ...
  mkPN : Str -> Str -> Str -> Str -> Gender -> PN = \nom,acc,dat,abl,g -> lin PN {
    s = table {Nom=>nom ; Acc=>acc ; Dat=>dat ; Gen=>dat ; Ablat=>abl} ;
    a = agrgP3 g Sg
  } ;
  mkPN : N -> PN = \n -> lin PN {
...
mkGN = overload {
  ...
  mkGN : Str -> Str -> Str -> Str -> Gender -> GN = \nom,acc,dat,abl,g -> lin GN {
    s = table {Nom=>nom ; Acc=>acc ; Dat=>dat ; Gen=>dat ; Ablat=>abl} ; g=g
  }
} ;
...
-- Exact adjective escape hatch. Ordinary productive classes are case-
-- syncretic, but A remains case-indexed so exceptional/pronominal lexical
-- items can preserve a real case contrast.
AForms : Type = {
  nomMascSg, nomMascPl, nomFemSg, nomFemPl,
  accMascSg, accMascPl, accFemSg, accFemPl,
  datMascSg, datMascPl, datFemSg, datFemPl,
  genMascSg, genMascPl, genFemSg, genFemPl,
  ablatMascSg, ablatMascPl, ablatFemSg, ablatFemPl : Str
} ;

mkAFull : AForms -> AdjClass -> A = \f,cls -> lin A {
  s = table {
    Nom => table {Masc=>table {Sg=>f.nomMascSg; Pl=>f.nomMascPl}; Fem=>table {Sg=>f.nomFemSg; Pl=>f.nomFemPl}} ;
    Acc => table {Masc=>table {Sg=>f.accMascSg; Pl=>f.accMascPl}; Fem=>table {Sg=>f.accFemSg; Pl=>f.accFemPl}} ;
    Dat => table {Masc=>table {Sg=>f.datMascSg; Pl=>f.datMascPl}; Fem=>table {Sg=>f.datFemSg; Pl=>f.datFemPl}} ;
    Gen => table {Masc=>table {Sg=>f.genMascSg; Pl=>f.genMascPl}; Fem=>table {Sg=>f.genFemSg; Pl=>f.genFemPl}} ;
    Ablat => table {Masc=>table {Sg=>f.ablatMascSg; Pl=>f.ablatMascPl}; Fem=>table {Sg=>f.ablatFemSg; Pl=>f.ablatFemPl}}
  } ;
  cls = cls
} ;

mkA = overload {
  mkA : Str -> A = regA;   -- nominative masculine singular
  mkA : Str -> Str -> A = reg2A ; -- + nominative masculine plural
  mkA : Str -> Str -> Str -> A = reg3A -- + nominative feminine singular validator
} ;

unartA : A -> A = \a -> a ** {cls=Unarticulated} ;
...
mkV = overload {
  mkV : Str -> V = regV;   -- present 1sg; compatibility/smart heuristic
  mkV : Str -> Str -> V = reg2V ; -- present 1sg, participle
  mkV : Str -> Str -> Str -> V = reg3V ; -- present 1sg, aorist 1sg, participle
  mkV : Str -> Str -> Str -> Str -> V = reg4V -- + imperfect 1sg validator
} ;
...
mkIP : Str -> IP = \s -> lin IP {
  s = table {Nom=>s; Acc=>s; Dat=>s; Gen=>s; Ablat=>s} ;
  a = agrgP3 Masc Sg
} ;
