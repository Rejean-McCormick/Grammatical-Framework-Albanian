...
param
  ...
  -- Grammatical case is kept distinct even when surface morphology is
  -- syncretic.  Standard Albanian has a grammatical genitive distinct from
  -- the dative, while the noun ending itself is syncretic in the evidence
  -- supplied for this workstream.
...
param
  -- Preserve agreement gender in the plural.  This is required for
  -- ambigeneric nouns whose agreement gender differs by number.
...
oper
  ...
  nounGender : Noun -> Number -> Gender = \n,num -> n.g ! num ;

  mkNounGenders :
    (_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_ : Str) -> Gender -> Gender -> Noun =
    \f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,f16,gsg,gpl ->
      { s = table {
              Indef => table {
  ...
  mkNoun : (_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_ : Str) -> Gender -> Noun =
    \f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,f16,g ->
      mkNounGenders f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 f12 f13 f14 f15 f16 g g ;

  setNounGender : Noun -> Gender -> Gender -> Noun = \n,gsg,gpl ->
    n ** {g = table {Sg => gsg ; Pl => gpl}} ;


oper
  -- Ordinary Albanian adjective classes are case-syncretic, but the RGL A
  -- category also contains a small number of case-conditioned lexical items
  -- (notably pronominal/quantificational adjectives such as cilido_A). Keep
  -- Case in the representation so those items remain exact. Productive A
  -- classes deliberately keep ordinary endings syncretic across case.
  Adj : Type = {s : Case => Gender => Number => Str ; cls : AdjClass} ;

  adjForm : Adj -> Case -> Gender -> Number -> Str = \a,c,g,n -> a.s ! c ! g ! n ;

  adjNeedsNyje : Adj -> Bool = \a -> case a.cls of {
    Articulated => True ;
    Unarticulated => False
  } ;

  mkAdj : (_,_,_,_ : Str) -> AdjClass -> Adj = \ms,mp,fs,fp,cls -> {
    s = \\_ => table {
  ...
  -- Migration helper for the historical 4-case records. Grammatical Gen is
  -- syncretic with Dat unless a lexical constructor supplies a special form.
  mkAdjLegacy : (_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_ : Str) -> Bool -> Adj =
    \f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,f16,clit -> {
      s = table {
        Nom => table {Masc => table {Sg=>f1; Pl=>f2}; Fem=>table {Sg=>f3; Pl=>f4}} ;
        Acc => table {Masc => table {Sg=>f5; Pl=>f6}; Fem=>table {Sg=>f7; Pl=>f8}} ;
        Dat => table {Masc => table {Sg=>f9; Pl=>f10}; Fem=>table {Sg=>f11; Pl=>f12}} ;
        Gen => table {Masc => table {Sg=>f9; Pl=>f10}; Fem=>table {Sg=>f11; Pl=>f12}} ;
        Ablat => table {Masc => table {Sg=>f13; Pl=>f14}; Fem=>table {Sg=>f15; Pl=>f16}}
      } ;
      cls = case clit of {True => Articulated ; False => Unarticulated}
    } ;
...
oper
  ...
  mkPron : (_,_,_,_,_,_ : Str) -> GenNum -> Person -> Pron =
    \nom,acc,dat,ablat,accC,datC,gn,p ->
      { s = table {Nom => nom ; Acc => acc ; Dat => dat ; Gen => dat ; Ablat => ablat} ;
        acc_clit = accC ;
        dat_clit = datC ;
        a = {gn = gn ; p = p} ;
        isPron = True
...
oper
  ...
  -- Auxiliary service for analytic anterior forms.  The lexical verb carries
  -- only the morphologically selected auxiliary class; clause realization
  -- chooses the finite form here.  Non-active morphology selects jam.
  ...
  selectPerfectAux : MorphVoice -> PerfectAux -> PerfectAux = \voice,aux ->
    case voice of {NonActiveMorph => AuxJam ; ActiveMorph => aux} ;

  perfectAuxFinite : PerfectAux -> ParamX.Tense -> Number -> Person -> Str = \aux,t,n,p ->
    case aux of {AuxKam => haveAux ! t ! n ! p ; AuxJam => beAux ! t ! n ! p} ;

  perfectAuxSubj : PerfectAux -> Number -> Person -> Str = \aux,n,p ->
    case aux of {AuxKam => haveSubj ! n ! p ; AuxJam => beSubj ! n ! p} ;
