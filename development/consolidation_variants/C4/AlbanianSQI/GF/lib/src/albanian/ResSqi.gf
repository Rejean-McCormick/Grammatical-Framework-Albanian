...
param
  -- Albanian morphology keeps the ordinary synthetic pasts distinct from
  -- the analytic perfect.  Public RGL TPast is mapped separately below.
  Tense = Pres | Perfect | Imperfect | Aorist ;

  -- Morphological voice belongs to the lexical/morphological verb.  It is
  -- deliberately distinct from semantic/syntactic voice use (passive,
  -- reflexive, reciprocal) carried by VP.
...
oper
  ...
  VP : Type = {
    ...
    -- Structured realization state.  cl is the ordinary weak-pronoun
    -- cluster; subjcl is the corresponding të+cluster contraction (të,
    -- ta, t'i, t'u...).  Particles such as do/po/mos are owned by the
    -- clause realizer and are never stored by ProgrVP or ReflVP.
    cl : Str ;
    subjcl : Str ;
    post : Agr => Str ;
...
oper
  ...
  -- Active perfect auxiliary.  ParamX.Past deliberately selects the
  -- imperfect auxiliary (kisha...) for the neutral RGL past-anterior.
  ...
  -- Non-active compound tenses change the auxiliary from kam to jam.
  -- The past-anterior counterpart of kisha is therefore isha; the
  -- aorist-auxiliary series qeshë/qe/... is exposed separately below.
  ...
  -- Aorist auxiliaries for the explicitly Albanian anterior-past series
  -- (pata/qeshë + participle).  This is not the common RGL TPast+AAnter.
  haveAorAux : Number => Person => Str = table {
    Sg => table {P1 => "pata" ; P2 => "pate" ; P3 => "pati"} ;
    Pl => table {P1 => "patëm" ; P2 => "patët" ; P3 => "patën"}
  } ;

  beAorAux : Number => Person => Str = table {
    Sg => table {P1 => "qeshë" ; P2 => "qe" ; P3 => "qe"} ;
    Pl => table {P1 => "qemë" ; P2 => "qetë" ; P3 => "qenë"}
  } ;
