...
-- TAM particles and auxiliaries are intentionally NOT assembled here; the
-- unique finite realization boundary is ClauseSqiRes.realizeVP.
concrete VerbSqi of Verb = CatSqi **
  ...
  oper
    ...
    -- VV complements in Standard/Tosk Albanian are finite subjunctives.  A
    -- progressive subordinate strategy is intentionally not synthesized as
    -- *të po ...*: that compatibility remains closed until evidence exists.
    vpiStr : VP -> Agr -> Str = \vp,a -> case vp.progressivity of {
      NeutralProgressive =>
        vp.subjcl ++ vp.subjunctive ! agrNumber a ! a.p ++ vp.post ! a ;
      Progressive => nonExist
    } ;

    slashVpiStr : VPSlash -> Agr -> Str = \vp,a -> case vp.progressivity of {
      NeutralProgressive =>
        vp.subjcl ++ vp.subjunctive ! agrNumber a ! a.p ++ vp.post ! a ;
      Progressive => nonExist
    } ;
  ...
  lin
    ...
    -- Common RGL ReflVP is the weak reflexive-clitic construction.  This is
    -- semantically reflexive but does not relabel the lexical verb as
    -- morphologically non-active; lexical non-active verbs are separate V
    -- paradigms and therefore select jam independently in compound tenses.
    ReflVP sl = (addClitic (slashToVP sl) "u" "t'u") ** {
      voiceUse = ReflexiveUse
    } ;

    -- Analytic passive: jam + lexical participle.  Keeping the copula as the
    -- VP head makes higher anteriority compositional (kam qenë + participle)
    -- instead of conflating analytic passive with morphological non-active.
