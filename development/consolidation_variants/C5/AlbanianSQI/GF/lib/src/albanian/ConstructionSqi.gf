...
concrete ConstructionSqi of Construction = CatSqi **
  ...
  lin
    ...
    n_units_AP c cn a = {
      s=\\sp,cas,g,n=>c.s ++ nounForm cn Indef cas Pl ++ adjForm a cas g n
    } ;

    n_units_of_NP c cn np = lin NP {
      s=\\cas=>c.s ++ nounForm cn Indef cas Pl ++ "prej" ++ np.s!Ablat; a=agrgP3 Masc Pl; isPron=False
    } ;

    n_unit_CN c unit cn = {
      s=\\sp,cas,n=>c.s ++ nounForm unit Indef cas Sg ++ nounForm cn sp cas n;
      g=cn.g
    } ;
    ...
    few_X_short_of_Y np x y = {
      s=np.s!Nom ++
        ...
        "pak" ++ nounForm x Indef Acc Pl ++
        "më pak se" ++ nounForm y Indef Acc Sg
    } ;
