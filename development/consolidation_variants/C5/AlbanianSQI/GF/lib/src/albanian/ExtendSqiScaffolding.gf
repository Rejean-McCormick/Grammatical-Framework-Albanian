...
oper
  ...
  sc_GenRP : Num -> CN -> RP = \num,cn -> lin RP {
    s=\\c,gn =>R.nounForm cn R.Def c num.n ++
      R.nyje!R.Def!c!cn.g!num.n ++ relativeGenitive gn
  } ;

  sc_GenModNP : Num -> NP -> CN -> NP = \num,np,cn -> lin NP {
    s=\\c =>R.nounForm cn R.Def c num.n ++ R.nyje!R.Def!c!cn.g!num.n ++ np.s!R.Gen ; a=R.agrgP3 cn.g num.n ; isPron=False
  } ;

  sc_GenModIP : Num -> IP -> CN -> IP = \num,ip,cn -> lin IP {
    s=\\c =>R.nounForm cn R.Def c num.n ++ R.nyje!R.Def!c!cn.g!num.n ++ ip.s!R.Gen ;
  ...
  -- The common Extend API exposes stranding constructors, but Standard
  -- Albanian does not license preposition stranding as the neutral strategy.
  -- Preserve API coverage by neutralizing both constructors to pied-piping.
  ...
  -- EmptyRelSlash is safe for an unprepositioned Acc gap (që strategy).
  -- Any governed/prepositional gap must use an overt relative strategy rather
  -- than silently generating English-like stranding.
  sc_EmptyRelSlash : ClSlash -> RCl = \slash -> lin RCl {
    s=\\_,t,a,p =>case <slash.c2.s : Str> of {
      "" => case slash.c2.c of {
        R.Acc => "që" ++ slash.s!t!a!p ;
        _ => Predef.error "Albanian EmptyRelSlash requires an overt case-bearing relative strategy for non-Acc gaps"
      } ;
      _ => Predef.error "Albanian EmptyRelSlash does not strand prepositions; use PiedPipingRelSlash"
    }
  } ;
  ...
  sc_SubjunctRelCN : CN -> RS -> CN = \cn,rs -> cn ** {
    s=\\sp,c,n =>R.nounForm cn sp c n ++ rs.s!(R.agrgP3 cn.g n)
  } ;
