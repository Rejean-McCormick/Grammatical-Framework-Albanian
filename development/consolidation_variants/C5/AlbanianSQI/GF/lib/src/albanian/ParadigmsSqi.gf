...
mkPN = overload {
  ...
  mkPN : N -> PN = \n -> lin PN {
    s = \\c => nounForm n Def c Sg ; a = agrgP3 n.g Sg
  }
} ;
...
mkGN = overload {
  ...
  mkGN : Str -> Str -> Str -> Str -> Gender -> GN = \nom,acc,dat,abl,g -> lin GN {
    s = table {Nom=>nom ; Acc=>acc ; Gen=>dat ; Dat=>dat ; Ablat=>abl} ; g=g
  }
} ;
...
mkPron : (nom,acc,gendat,ablat : Str) -> GenNum -> Person -> Pron =
  \nom,acc,gendat,ablat,gn,p -> lin Pron
     {s = table {Nom=>nom; Acc=>acc; Gen=>gendat; Dat=>gendat; Ablat=>ablat};
      a = {gn=gn; p=p};
      isPron = True
     } ;
...
mkPrep = overload {
  mkPrep : Str -> Prep = \s -> lin Prep {s=s; c=Acc; kind=OrdinaryCompl} ;
  mkPrep : Str -> Case -> Prep = \s,c -> lin Prep {s=s; c=c; kind=OrdinaryCompl}
} ;
noPrep : Prep = lin Prep {s=[]; c=Acc; kind=OrdinaryCompl} ;
