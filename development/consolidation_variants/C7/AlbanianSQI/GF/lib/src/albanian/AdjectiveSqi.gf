...
concrete AdjectiveSqi of Adjective = CatSqi **
  ...
  lin
    ...
    -- SC already owns its embedding morphology (që/se/të/etc.).  Do not add a
    -- second complementizer here: SentAP (EmbedS s) must not yield "që që".
    SentAP ap sc = {
      s = \\spec,c,g,n => ap.s ! spec ! c ! g ! n ++ sc.s
    } ;
