...
oper
  -- Compatibility fallback for genuinely subjectless/fragments.  This is not
  -- a linguistic default for all Albanian clauses.
  fallbackAgr : Agr = {gn = GSg Masc ; p = P3} ;
  defaultAgr : Agr = fallbackAgr ;

  -- Supplied Albanian evidence attests feminine singular agreement for the
  -- impersonal/extraposed family with a clausal subject (e.g. "është e ...").
  -- Keep it separate from fallbackAgr so it cannot silently spread elsewhere.
  clausalSubjectAgr : Agr = {gn = GSg Fem ; p = P3} ;

  indicativeComplementizer : Str = "që" ;
  invariantRelativeMarker : Str = "që" ;
  polarQuestionMarker : Str = "a" ;

  embedIndicative : Str -> Str = \s -> indicativeComplementizer ++ s ;
  markPolarQuestion : Str -> Str = \s -> polarQuestionMarker ++ s ;
  markRelativeClause : Str -> Str = \s -> invariantRelativeMarker ++ s ;

  frontWh : Str -> Str -> Str = \wh,body -> wh ++ body ;

  frontGovernedIP : Compl -> (Case => Str) -> Str -> Str = \c,ip,body ->
    c.s ++ ip ! c.c ++ body ;

  frontGovernedRP : Compl -> (Case => GenNum => Str) -> GenNum -> Str -> Str =
    \c,rp,gn,body -> c.s ++ rp ! c.c ! gn ++ body ;

  -- Assembly helper for an already-finalized predicate table (VPS/VPI-like
  -- records).  It does not duplicate TAM or clitic realization.
  assembleFinalPredicate : Str -> Agr -> (Agr => Str) -> Str = \subj,a,pred ->
    subj ++ pred ! a ;
