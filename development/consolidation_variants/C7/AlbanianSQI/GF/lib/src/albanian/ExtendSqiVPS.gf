...
oper
  ...
  vps_PredVPS : NP -> VPSRec -> S = \np,x -> lin S {
    s=assembleFinalPredicate (np.s!R.Nom) np.a x.s
  } ;
  vps_SQuestVPS : NP -> VPSRec -> QS = \np,x -> lin QS {
    s=markPolarQuestion (assembleFinalPredicate (np.s!R.Nom) np.a x.s)
  } ;
  vps_QuestVPS : IP -> VPSRec -> QS = \ip,x -> lin QS {
    s=assembleFinalPredicate (ip.s!R.Nom) ip.a x.s
  } ;
  vps_RelVPS : RP -> VPSRec -> RS = \rp,x -> lin RS {
    s=\\a =>assembleFinalPredicate (rp.s!R.Nom!a.gn) a x.s
  } ;
