concrete IdiomSqi of Idiom = CatSqi ** open Prelude, ParamX, ResSqi, ClauseSqiRes, (I=IrregSqi) in {
  ...
  lin
    ImpersCl vp = {s=\\t,a,p=>realizeVP vp t a p fallbackAgr} ;
    GenericCl vp = {s=\\t,a,p=>mkClause "njeriu" fallbackAgr vp t a p} ;

    CleftNP np rs = {
      s=\\t,a,p=>realizeVP cleftCop t a p np.a ++ np.s!Nom ++ rs.s!np.a
    } ;
    CleftAdv adv s = {
      s=\\t,a,p=>realizeVP cleftCop t a p fallbackAgr ++ adv.s ++ embedIndicative s.s
    } ;
