...
lin
  ...
  PredSCVP sc vp = {
    s = \\t,ant,pol => mkClause sc.s clausalSubjectAgr vp t ant pol
  } ;
  ...
  SlashVS np vs sslash = {
    s = \\t,ant,pol =>
      let vp : VP = appendVP (emptyVP vs) (\\_ => embedIndicative sslash.s)
  ...
  EmbedS s = {s = embedIndicative s.s} ;
  -- Embedded interrogative morphology/order is still evidence-open.  Preserve
  -- the existing QS surface instead of inventing an English-like whether/if.
  EmbedQS qs = {s = qs.s} ;
  EmbedVP vp = {s = realizeSubjVP vp Pos fallbackAgr} ;
