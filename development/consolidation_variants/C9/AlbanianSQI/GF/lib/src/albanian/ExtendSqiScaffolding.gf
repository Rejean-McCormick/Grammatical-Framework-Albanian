...
oper
  ...
  -- the canonical strategy.  Retain the common RGL API, but neutralize the
  -- stranding distinction to an Albanian preposition-first realization.
  ...
  -- EmptyRelSlash has no RP argument, so an overt relative form must be
  -- synthesized.  Current Agr preserves singular gender but collapses plural
  -- gender; masculine plural is therefore the explicit compatibility default
  -- until the accepted richer agreement migration lands in the core.
  sc_EmptyRelSlash : ClSlash -> RCl = \slash -> lin RCl {
    s=\\agr,t,a,p =>case Predef.eqStr slash.c2.s [] of {
      -- With no overt adposition, use the ordinary invariant relative marker.
      Predef.PTrue => "që" ++ slash.s!t!a!p ;
      -- When the governed adposition must remain visible, supply an overt
      -- declined relative pronoun after it instead of stranding the adposition.
      Predef.PFalse => slash.c2.s ++ sc_RelativeCili slash.c2.c agr.gn ++ slash.s!t!a!p
    }
  } ;
  ...
  sc_RelativeCili : R.Case -> R.GenNum -> Str = \c,gn ->
    case gn of {
      R.GSg R.Masc => case c of {
        R.Nom => "i cili" ; R.Acc => "të cilin" ; R.Dat => "të cilit" ; R.Ablat => "të cilit"
      } ;
      R.GSg R.Fem => case c of {
        R.Nom => "e cila" ; R.Acc => "të cilën" ; R.Dat => "së cilës" ; R.Ablat => "së cilës"
      } ;
      R.GPl => case c of {
        R.Nom => "të cilët" ; R.Acc => "të cilët" ; R.Dat => "të cilëve" ; R.Ablat => "të cilëve"
      }
    } ;
